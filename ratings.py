...
def rate_recipe(recipe_id, user_id, rate):
    if rate < 1 or rate > 5:
        return False
    result = db.query("""SELECT id FROM ratings
                    WHERE recipe_id = ? AND user_id = ?""",
                      [recipe_id, user_id])
    con = db.connect()
    try:
        if len(result) == 0:
            con.execute("""INSERT INTO ratings (recipe_id, user_id, rate)
                        VALUES (?, ?, ?)""",
                        [recipe_id, user_id, rate])
            con.commit()
        else:
            con.execute("""UPDATE ratings SET rate = ?
                        WHERE recipe_id = ? AND user_id = ?""",
...
def average_by_recipe_id(recipe_id):
    avg = db.query("""SELECT AVG(rate) AS avg 
...
def count_ratings_by_user_id(user_id):
    return db.query("""SELECT COUNT(ratings.id) AS count
                    FROM ratings, recipes
                    WHERE ratings.recipe_id = recipes.id
                    AND recipes.creator_id = ?""",
        [user_id])[0]["count"]
...
def get_rating_by_user_id_and_recipe_id(user_id, recipe_id):
    result = db.query("""SELECT rate FROM ratings
